  ...
  if (base) {
    if (/^[A-Za-z]:/.test(normalised) || normalised.startsWith('/')) {
      ...
      for (let i = 0; i < parts.length; i++) {
        const candidate = path.join(base, parts.slice(i).join('/'))
...
// GET /api/images/:id/file — serves image file for both local and S3
...
router.get('/:id', async (req, res) => {
  try {
    const [rows] = await db.query(`${baseSelect()} WHERE i.id = ?`, [req.params.id])
    if (!rows.length) return res.status(404).json({ error: 'Image not found' })
    res.json(rows[0])
  } catch (err) {
    console.error(err)
...
// GET /api/images/:id/similar — returns similar images from all cluster types
router.get('/:id/similar', async (req, res) => {
  try {
    const [clusters] = await db.query(
      `SELECT dc.cluster_id, dc.cluster_type
       FROM duplicate_clusters dc
       WHERE dc.image_id = ? AND dc.cluster_id IS NOT NULL
       AND (
         SELECT COUNT(*) FROM duplicate_clusters
         WHERE cluster_id = dc.cluster_id
         AND cluster_type = dc.cluster_type
       ) > 1`,
      [req.params.id]
    )
    if (!clusters.length) return res.json([])

    const seenIds = new Set([Number(req.params.id)])
    const allRows = []
    ...
    for (const cluster of clusters) {
      const [rows] = await db.query(`
        SELECT
          ...
          COALESCE(c_scene.human_value, a.scene_type)   AS scene_type,
          COALESCE(c_people.human_value, a.people_count) AS people_count,
          a.scene_confidence, a.people_confidence,
          a.tags, a.categories, a.caption, a.model_name, a.is_verified,
          q.sharpness_score, q.exposure_score, q.overall_score, q.is_best_in_group,
          dc2.cluster_id, dc2.is_representative, dc2.similarity_score
        FROM images i
        LEFT JOIN exif_metadata      e        ON e.image_id  = i.id
        LEFT JOIN ai_tags            a        ON a.image_id  = i.id
        LEFT JOIN quality_scores     q        ON q.image_id  = i.id
        LEFT JOIN duplicate_clusters dc2      ON dc2.image_id = i.id AND dc2.cluster_type = ?
        LEFT JOIN corrections        c_scene  ON c_scene.image_id  = i.id AND c_scene.field_name  = 'scene_type'
        LEFT JOIN corrections        c_people ON c_people.image_id = i.id AND c_people.field_name = 'people_count'
        WHERE dc2.cluster_id = ?
          AND dc2.cluster_type = ?
          AND i.id != ?
        ORDER BY q.overall_score DESC
      `, [cluster.cluster_type, cluster.cluster_id, cluster.cluster_type, req.params.id])
    ...
    allRows.sort((a, b) => (b.overall_score || 0) - (a.overall_score || 0))
    res.json(allRows)
  } catch (err) {
    console.error(err)
    res.status(500).json({ error: err.message })
